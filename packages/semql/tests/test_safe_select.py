"""Tests for ``semql.safe.is_safe_select`` — belt-and-braces guard.
...
def test_select_with_union_is_safe() -> None:
    ...
    assert is_safe_select("SELECT 1 UNION ALL SELECT 2") is True


def test_select_with_cte_is_safe() -> None:
    assert is_safe_select("WITH t AS (SELECT 1 AS x) SELECT * FROM t") is True
...
def test_compiled_sql_round_trip_is_safe() -> None:
    ...
    assert is_safe_select(out.sql) is True
...
def test_malformed_sql_rejected() -> None:
    assert is_safe_select("SELECT FROM WHERE") is False
...
def test_dialect_kwarg_threads_to_parser() -> None:
    ...
    assert (
        is_safe_select("SELECT count(*) FROM t WHERE x = {p0:String}", dialect="clickhouse") is True
    )
