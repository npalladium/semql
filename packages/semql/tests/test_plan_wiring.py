...
"""Wire the LogicalPlan into the compile pipeline.
...
def test_time_aggregation_emits_truncated_bucket(catalog: dict) -> None:
    ...
    Before this change: emission reads ``q.time_dimension.granularity``
    directly. After it: it reads ``self.plan.time_window.granularity``.
...
def test_time_window_granularity_via_plan_only(catalog: dict) -> None:
    ...
    This is the structural property this change enforces: the plan is the
...
def test_emission_reads_time_window_from_plan(catalog: dict) -> None:
    ...
    ``_CompileEnv`` builds the plan *before* the time-block,
